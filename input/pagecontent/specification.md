...
In the event that the prior authorization cannot be evaluated and a final response returned within the required timeframe, a response in which one or more of the requested authorization items are 'pended' will be returned. §spec-8^client:A subscription-based mechanism **SHALL** be used by the client to be informed of updates to the authorization.§ As well, the client (or other interested systems - e.g., patient, caregiver or performing provider systems) can use the [Claim inquire operation](OperationDefinition-Claim-inquiry.html) to query for the final results. Since the Inquiry does not return all information while a request is pended, it should only be used after the request is finalized.
...
To communicate updates clients will use the $submit operation to provide a [PAS Request Bundle](StructureDefinition-profile-pas-request-bundle.html) with a [PAS Claim Update](StructureDefinition-profile-claim-update.html) instance as the first entry. §spec-65^client:The Claim that is being updated **SHALL** be referenced in the Claim.related.claim element and included in the Bundle.§ §spec-66^client:If that Claim instance is itself a Claim Update, its referenced Claim **SHALL NOT** be included.§ §spec-67^client:All other referenced resources **SHALL** be included in the Bundle.§  §spec-68^client:To cancel the entire request, clients **SHALL** send a [certificationType extension](StructureDefinition-extension-certificationType.html) with a code of 3 (Cancel) in the Claim.extension element of a PAS Claim Update resource. No items are needed to be sent to cancel the entire authorization.§ §spec-69^client:When changing the details of the request, the Claim instance **SHALL** contain all item and supportingInfo entries from the original Claim and any previous update Claims with `sequence` values preserved, but with the current request details.§ Additionally, items and supportingInfo entries will be flagged with extensions in the following cases:
