...
* Implementers of this IG **SHOULD** support the endpoint discovery mechanism defined in the HRex specification to allow discovery of the endpoints used in this IG - specifically the davinci_pas_submission_endpoint which is used for $submit and $inquire operations.
...
For all of these cases, the update is requested by creating a *new* Bundle containing a *new* Claim resource with its own unique Claim.identifier and posting it using the $submit operation.  That resource will point to the previous Claim using the Claim.related element.  The relationship type **SHALL** be 'prior'.  The new claim resource instance will comply with the [Revised Prior Authorization](StructureDefinition-profile-claim-update.html) profile.  **NOTE**: The previous Claim.identifier is not specified on the X12 update.  The Trace Number on the update will be the new Claim.identifier.  As well, the previous authorization number and previous administration reference number that were returned in the response **SHOULD** be set on the new updated Claim.
