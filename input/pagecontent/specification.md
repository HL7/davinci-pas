...
In the event that the prior authorization cannot be evaluated and a final response returned within the required timeframe, a response in which one or more of the requested authorization items are 'pended' will be returned. §spec-8^client:A subscription-based mechanism **SHALL** be used by the client to be informed of updates to the authorization.§ As well, the client (or other interested systems - e.g., patient, caregiver or performing provider systems) can use the [Claim inquire operation](OperationDefinition-Claim-inquiry.html) to query for the final results.  
